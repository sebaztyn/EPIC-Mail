...
describe('MESSAGES ENDPOINTS', function () {
  ...
  describe('Checking for Errors and Empty Retrievals from server', function () {
    ...
    it('should return status code 200 when a request is made to retrieve messages from an empty INBOX', function (done) {
      _chai.default.request(_server.default).get('/api/v1/messages').set('Authorization', "Bearer ".concat(errorToken)).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
    ...
    it('should return status code 200 when a request is made to retrieve UNREAD messages for user with NO UNREAD message', function (done) {
      _chai.default.request(_server.default).get('/api/v1/messages/unread').set('Authorization', "Bearer ".concat(errorToken)).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
        expect(res.body).to.have.keys('status', 'message');
        expect(res.body).to.have.ownProperty('status').that.equals(200);
    ...
    it('should return status code 200 when a request is made to retrieve SENT messages for user with NO SENT message', function (done) {
      _chai.default.request(_server.default).get('/api/v1/messages/sent').set('Authorization', "Bearer ".concat(errorToken)).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
        expect(res.body).to.have.keys('status', 'message');
        expect(res.body).to.have.ownProperty('status').that.equals(200);
    ...
    it('should return an Error 404 when the wrong messageID is used as a parameter in retrieving a specific SENT message', function (done) {
      _chai.default.request(_server.default).get('/api/v1/messages/sent/1').set('Authorization', "Bearer ".concat(errorToken)).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
    ...
    it('should return an Error 404 when the wrong messageID is used as a parameter in retrieving a specific INBOX message', function (done) {
      _chai.default.request(_server.default).get('/api/v1/messages/30').set('Authorization', "Bearer ".concat(errorToken)).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
        expect(res.body).to.have.keys('status', 'error');
        expect(res.body).to.have.ownProperty('status').that.equals(404);
    ...
    it('should return an Error 404 when the wrong messageID is used as a parameter in DELETING a specific INBOX message', function (done) {
      _chai.default.request(_server.default).delete('/api/v1/messages/30').set('Authorization', "Bearer ".concat(errorToken)).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
        expect(res.body).to.have.keys('status', 'error');
        expect(res.body).to.have.ownProperty('status').that.equals(404);
    ...
    it('should return an Error 404 when the wrong messageID is used as a parameter in DELETING a specific SENT message', function (done) {
      _chai.default.request(_server.default).delete('/api/v1/messages/sent/30').set('Authorization', "Bearer ".concat(errorToken)).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
        expect(res.body).to.have.keys('status', 'error');
        expect(res.body).to.have.ownProperty('status').that.equals(404);
  ...
  before(function (done) {
    _chai.default.request(_server.default).post('/api/v1/messages').set('Authorization', "Bearer ".concat(userToken)).send({
  ...
  before(function (done) {
    _chai.default.request(_server.default).post('/api/v1/messages').set('Authorization', "Bearer ".concat(secondToken)).send({
      subject: 'Are you Ready',
      message: 'Chelsea is coming to town!!!!!!!',
      email: 'uche@yahoo.com'
    }).end(function (err, res) {
  ...
  it('should return all received messages when a call is made to the endpoint', function (done) {
    _chai.default.request(_server.default).get('/api/v1/messages').set('Authorization', "Bearer ".concat(secondToken)).end(function (err, res) {
      if (err) return done(err);
  ...
  it('should return specific value:UNREAD when a call is made to all received unread messages', function (done) {
    _chai.default.request(_server.default).get('/api/v1/messages/unread').set('Authorization', "Bearer ".concat(testToken)).end(function (err, res) {
      if (err) {
        done(err);
      }
  ...
  it('should return specific value:SENT when a call is made to all sent messages', function (done) {
    _chai.default.request(_server.default).get('/api/v1/messages/sent').set('Authorization', "Bearer ".concat(testToken)).end(function (err, res) {
      if (err) done(err);
      expect(res.body).to.have.keys('status', 'data');
      expect(res.body).to.have.ownProperty('status').that.equals(200);
      expect(res.body).to.have.ownProperty('data').to.be.an('array');
  ...
  it('should return a unique INBOX message matching the message ID', function (done) {
    _chai.default.request(_server.default).get('/api/v1/messages/1').set('Authorization', "Bearer ".concat(testToken)).end(function (err, res) {
      if (err) return done(err);
      expect(res.body).to.have.keys('status', 'data');
      expect(res.body).to.have.ownProperty('status').that.equals(200);
      expect(res.body).to.have.ownProperty('data').to.be.an('array');
  ...
  it('should return a unique SENT message matching the message ID', function (done) {
    _chai.default.request(_server.default).get('/api/v1/messages/sent/5').set('Authorization', "Bearer ".concat(testToken)).end(function (err, res) {
      if (err) return done(err);
      expect(res.body).to.have.keys('status', 'data');
      expect(res.body).to.have.ownProperty('status').that.equals(200);
      expect(res.body).to.have.ownProperty('data').to.be.an('array');
  ...
  it('should return a message stating that an INBOX message was deleted successfully', function (done) {
    _chai.default.request(_server.default).delete('/api/v1/messages/2').set('Authorization', "Bearer ".concat(testToken)).end(function (err, res) {
      if (err) done(err);
      expect(res.body).to.have.keys('status', 'data');
      expect(res.body).to.have.ownProperty('status').that.equals(200);
      expect(res.body).to.have.ownProperty('data').to.be.an('array');
  ...
  it('should return a message stating that a SENT message was deleted successfully', function (done) {
    _chai.default.request(_server.default).delete('/api/v1/messages/sent/2').set('Authorization', "Bearer ".concat(testToken)).end(function (err, res) {
      if (err) done(err);
      expect(res.body).to.have.keys('status', 'data');
      expect(res.body).to.have.ownProperty('status').that.equals(200);
      expect(res.body).to.have.ownProperty('data').to.be.an('array');
...
describe('GROUP ENDPOINTS', function () {
  describe('Checking for all groups where a user is a member or an admin', function () {
    ...
    it('should return status code 200 and a message stating that user should create or join a group when a request is made to retrieve ALL GROUPS by a user who has NEITHER JOINED OR CREATED A GROUP', function (done) {
      _chai.default.request(_server.default).get('/api/v1/groups').set('Authorization', "Bearer ".concat(errorToken2)).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
        expect(res.body).to.have.keys('status', 'message');
        expect(res.body).to.have.ownProperty('status').that.equals(200);
  ...
  describe('Checking if a group already exist at the point of creation', function () {
    ...
    before(function (done) {
      _chai.default.request(_server.default).post('/api/v1/groups').set('Authorization', "Bearer ".concat(errorToken3)).send({
    ...
    it('should return status code 404 when a request is made to send AN EMAIL TO GROUP WHERE THE ONLY MEMBER IS THE GROUP CREATOR', function (done) {
      _chai.default.request(_server.default).post('/api/v1/groups/4/messages').set('Authorization', "Bearer ".concat(errorToken3)).send({
        subject: 'Are you Available',
        message: 'Can we meetup at 5pm?'
      }).end(function (err, res) {
        if (err) return done(err);
    ...
    it('should return status code 403 when a request is made to DELETE a NON-EXISTENT group', function (done) {
      _chai.default.request(_server.default).delete('/api/v1/groups/30').set('Authorization', "Bearer ".concat(errorToken3)).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
        expect(res.body).to.have.keys('status', 'error');
    ...
    it('should return status code 404 when a request is made to DELETE a NON-EXISTENT USER from a group', function (done) {
      _chai.default.request(_server.default).delete('/api/v1/groups/4/users/6').set('Authorization', "Bearer ".concat(errorToken3)).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
        expect(res.body).to.have.keys('status', 'error');
        expect(res.body).to.have.ownProperty('status').that.equals(404);
    ...
    it('should return status code 400 when a request is made to CHANGE THE NAME AN EXISTING GROUP', function (done) {
      _chai.default.request(_server.default).patch('/api/v1/groups/4/name').set('Authorization', "Bearer ".concat(errorToken3)).send({
        name: ''
      }).end(function (err, res) {
        if (err) return done(err);
        expect(res.body).to.be.an('object');
    ...
    it('should return status code 400 when a request is made to ADD A USER TO GROUP with an unregistered email', function (done) {
      _chai.default.request(_server.default).post('/api/v1/groups/4/users/').set('Authorization', "Bearer ".concat(errorToken3)).send({
  ...
  before(function (done) {
    _chai.default.request(_server.default).post('/api/v1/groups/1/users/').set('Authorization', "Bearer ".concat(testToken)).send({
      email: 'Iheoma@yahoo.com'
    }).end(function (err, res) {
      if (err) return done(err);
      done();
  ...
  it('should return all groups created by a particular user when a request to the endpoint', function (done) {
    _chai.default.request(_server.default).get('/api/v1/groups').set('Authorization', "Bearer ".concat(testToken)).end(function (err, res) {
      if (err) return done(err);
      expect(res.body).to.have.keys('status', 'data');
      expect(res.body).to.have.ownProperty('status').that.equals(200);
      expect(res.body).to.have.ownProperty('data').to.be.an('array');
  ...
  it('should change the name of a group in the database successfully', function (done) {
    _chai.default.request(_server.default).patch('/api/v1/groups/1/name').set('Authorization', "Bearer ".concat(testToken)).send({
  ...
  it('should delete a PARTICULAR USER from the group members table successfully', function (done) {
    _chai.default.request(_server.default).delete('/api/v1/groups/1/users/1').set('Authorization', "Bearer ".concat(testToken)).end(function (err, res) {
      if (err) return done(err);
      expect(res.body).to.haveOwnProperty('status');
      expect(res.body).to.haveOwnProperty('status').that.is.a('number');
      expect(res.body).to.haveOwnProperty('data');
  ...
  it('should delete a user\'s group in the database successfully', function (done) {
    _chai.default.request(_server.default).delete('/api/v1/groups/2').set('Authorization', "Bearer ".concat(testToken)).end(function (err, res) {
      if (err) return done(err);
      expect(res.body).to.haveOwnProperty('status');
      expect(res.body).to.haveOwnProperty('status').that.is.a('number');
      expect(res.body).to.haveOwnProperty('data');
