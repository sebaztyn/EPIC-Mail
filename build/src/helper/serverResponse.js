...
var userResponse = function userResponse(response, token, statusValue) {
  ...
  var statusKey = values[0],
      dataKey = values[1],
      dataValue = values[2];
  return response.header('authorization', token).status(statusValue).json((_response$header$stat = {}, _defineProperty(_response$header$stat, statusKey, statusValue), _defineProperty(_response$header$stat, dataKey, dataValue), _response$header$stat));
};
